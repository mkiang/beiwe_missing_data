...
For our paper, *Sociodemographic  Characteristics of Missing Data in Digital Phenotyping*, in addition to our main model presented in the manuscript, we ran five additional models for both GPS and accelerometer non-collection as described in the Supplementary Information. Below, we first provide model comparison metrics (e.g., LOO and WAIC). We then provide the exact output of each model as well as the null model (i.e., no hierarchy and no sociodemographic covariates) along with model fit metrics. Session information is presented at the end. 

For details, please see the paper and it's Supplementary Information. We note that when presenting random effect estimates, we remove the row names out of an abundance of caution. Though user IDs are randomly assigned and study names are generic, combined with other (i.e., external) data, it may be possible to derive more information about participants and leaad to re-identification. The removal of random identifiers does not affect our results. 
...
### Model output for accelerometer Model 1 (primary model)
#### Output
...
### Model output for GPS Model 1 (primary model)
#### Output
```{r, warning=FALSE, error=FALSE, message=FALSE}
gps_m1u_dem
```
